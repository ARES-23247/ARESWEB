...
const _profilesRouter = new OpenAPIHono<AppEnv>();
...
_profilesRouter.use("*", async (c, next) => {
...
_profilesRouter.use("/team-roster", rateLimitMiddleware(100, 60));
_profilesRouter.use("/:userId", rateLimitMiddleware(100, 60));

// Apply persistent rate limiting to write routes
_profilesRouter.use("/update-me", persistentRateLimitMiddleware(10, 60));
_profilesRouter.use("/avatar", persistentRateLimitMiddleware(15, 60));
...
// Current User Routes
_profilesRouter.use("/me", ensureAuth);
_profilesRouter.use("/update-me", ensureAuth);
_profilesRouter.use("/avatar", ensureAuth);

export const profilesRouter = _profilesRouter
    ...
    .openapi(getTeamRosterRoute, async (c) => {
      const db = getDb(c);
    ...
    .openapi(getPublicProfileByIdRoute, async (c) => {
