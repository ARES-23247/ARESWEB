...
const _entitiesRouter = new OpenAPIHono<AppEnv>();

_entitiesRouter.use("*", ensureAuth);

export const entitiesRouter = _entitiesRouter
    .openapi(getEntityLinksRoute, async (c) => {
        const db = getDb(c);
    ...
    .openapi(saveEntityLinkRoute, async (c) => {
        const db = getDb(c);
    ...
    .openapi(deleteEntityLinkRoute, async (c) => {
        const db = getDb(c);
