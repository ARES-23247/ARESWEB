...
const _tbaRouter = new OpenAPIHono<AppEnv>();
...
_tbaRouter.use("*", ensureAuth);
_tbaRouter.use("*", rateLimitMiddleware(30, 60));

// Apply Cloudflare Edge Caching (stale-while-revalidate)
_tbaRouter.use("*", cache({
...
export const tbaRouter = _tbaRouter
    .openapi(getRankingsRoute, async (c) => {
    ...
    .openapi(getMatchesRoute, async (c) => {
        const params = c.req.valid("param");
        const { eventKey } = params;
        if (!/^[a-zA-Z0-9]+$/.test(eventKey)) {
          throw new ApiError("Invalid eventKey", 400);
