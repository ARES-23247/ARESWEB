...
import { AppEnv, ensureAdmin, rateLimitMiddleware, logAuditAction, getDb, ApiError, requireAuth } from "../middleware";

import * as financeRoutes from "../../../shared/routes/finance";

const _financeRouter = new OpenAPIHono<AppEnv>();

// All finance routes require admin access (admin role, mentor, or coach memberType)
...
export const financeRouter = _financeRouter
    .openapi(financeRoutes.getSummaryRoute, async (c) => {
    ...
    .openapi(financeRoutes.listPipelineRoute, async (c) => {
        const query = c.req.valid("query");
        const { seasonId } = query;
        const db = getDb(c);
    ...
    .openapi(financeRoutes.deletePipelineRoute, async (c) => {
    ...
    .openapi(financeRoutes.listTransactionsRoute, async (c) => {
        const query = c.req.valid("query");
    ...
    .openapi(financeRoutes.deleteTransactionRoute, async (c) => {
        const params = c.req.valid("param");
        const { id } = params;
        const db = getDb(c);
        const tx = await db
