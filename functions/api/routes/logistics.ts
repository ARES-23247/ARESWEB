...
const _logisticsRouter = new OpenAPIHono<AppEnv>();
...
_logisticsRouter.use("/admin/*", ensureAdmin);

export const logisticsRouter = _logisticsRouter
    .openapi(getLogisticsSummaryRoute, async (c) => {
      const db = getDb(c);
    ...
    .openapi(exportLogisticsEmailsRoute, async (c) => {
      const db = getDb(c);
