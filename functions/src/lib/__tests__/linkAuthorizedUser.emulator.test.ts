...
describe.skipIf(!emulatorHost)("linkAuthorizedUserByEmail (Firestore emulator)", () => {
  ...
  it("re-keys a pre-authorized invite to the signed-in uid exactly once", async () => {
    ...
    const linked = await linkAuthorizedUserByEmail({ uid, email, emailVerified: true });
    ...
    const linkedAgain = await linkAuthorizedUserByEmail({ uid, email, emailVerified: true });
    expect(linkedAgain).toBe(false);
    const count = await adminDb.collection("authorized_users").where("email", "==", email).get();
  ...
  it("refuses to link an unverified email to someone else's pre-authorization", async () => {
    ...
    const linked = await linkAuthorizedUserByEmail({ uid: attackerUid, email, emailVerified: false });
    expect(linked).toBe(false);

    const attackerDoc = await adminDb.collection("authorized_users").doc(attackerUid).get();
  ...
  it("drops an archived pre-authorization instead of inheriting it", async () => {
    ...
    expect((await adminDb.collection("authorized_users").doc(uid).get()).exists).toBe(false);
