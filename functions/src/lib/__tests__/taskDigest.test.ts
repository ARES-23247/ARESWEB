...
describe("task due-date digest", () => {
  ...
  it("posts one digest and excludes completed, archived, deleted, and undated tasks", async () => {
    ...
    expect(content).toContain("Overdue work");
  ...
  it("skips posting entirely when no task is due within the window", async () => {
    queryGet.mockResolvedValue({
      docs: [taskDoc("task_later", { title: "Later", status: "todo", archived: false, isDeleted: 0, dueDate: "2026-09-20", assignees: [] })],
    });
    await expect(sendTaskDueDigest(new Date("2026-08-17T12:00:00Z"))).resolves.toBe(true);
