  ...
  private receive(room:Room,seat:number,m:ClientMessage) {
    ...
    if(m.type==="input"){
      ...
      if(!i||![i.x,i.y,i.turn,i.speed].every(Number.isFinite)||Math.abs(i.x)>2||Math.abs(i.y)>2||Math.abs(i.turn)>2||i.speed<2||i.speed>5.8
        ||[i.intake,i.shoot,i.release].some(v=>typeof v!=="boolean")||[i.aimHive,i.aimFlower,i.deposit,i.aim,i.lockOn,i.shootHeld,i.depositHeld].some(v=>v!==undefined&&typeof v!=="boolean")
