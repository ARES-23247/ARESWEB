...
export const updateTournamentMatchSchema = z
  .object({
    ...
    opponents: z.array(boundedText(20).min(1)).min(1).max(6).optional(),
    scoreSelf: z.number().int().min(0).max(10000).nullable().optional(),
    scoreOpponent: z.number().int().min(0).max(10000).nullable().optional(),
    result: z.enum(["won", "lost", "tie", "upcoming"]).optional(),
  ...
  try {
    ...
    if (
      ...
      return next(
        new ApiError(
          403,
          "Forbidden: Tournament changes require admin or coach access",
        ),
      );
    }
...
function tournamentDto(id: string, data: TournamentRecord) {
  ...
  const derivedStatus =
    date && date >= new Date().toISOString().slice(0, 10) ? "upcoming" : "past";
  return {
    id,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const tournaments = snapshot.docs
      .map((document) =>
        tournamentDto(document.id, document.data() as TournamentRecord),
      )
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const limitValue = Math.min(
      250,
      Math.max(1, Number.isFinite(requestedLimit) ? requestedLimit : 250),
    );
    const snapshot = await adminDb
    ...
    const matches = snapshot.docs
      ...
      .map((document) =>
        matchDto(document.id, document.data() as TournamentMatchRecord),
      )
      .sort((a, b) =>
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      tournament: tournamentDto(
        snapshot.id,
        snapshot.data() as TournamentRecord,
      ),
    });
  }),
);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      tournament: tournamentDto(snapshot.id, {
        ...snapshot.data(),
        ...updateData,
      }),
    });
...
router.post(
  ...
  ensureAuth,
  ensureAdminOrCoach,
  validate(createTournamentMatchSchema),
  asyncHandler(async (req, res) => {
    await getActiveTournament(req.params.id);
    ...
    res
      .status(201)
      .json({ success: true, match: matchDto(document.id, matchData) });
  }),
);

router.put(
  "/:id/matches/:matchId/completion",
  ensureAuth,
  ensureAdminOrCoach,
