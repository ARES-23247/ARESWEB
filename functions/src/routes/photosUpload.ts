...
router.post(
  "/upload-unified",
  ensureTeamMember,
  uploadUnifiedLimiter,
  asyncHandler(async (req: AuthenticatedRequest, res) => {
    const { fileBase64, filename, mimeType, albumId, runAiLabeling } =
      req.body as {
        fileBase64: string;
        filename: string;
        mimeType: string;
    ...
    if (!fileBase64 || !filename || !mimeType) {
      throw new ApiError(
        400,
        "Missing required fields: fileBase64, filename, mimeType",
      );
    }
    ...
    if (
      ...
      throw new ApiError(
        400,
        "Upload a JPEG, PNG, or WebP image with a filename under 180 characters.",
      );
    }
    if (albumId && !/^[A-Za-z0-9_-]{1,200}$/.test(albumId))
      throw new ApiError(400, "Invalid album ID.");
    if (albumId) {
      const album = await adminDb.collection("albums").doc(albumId).get();
      if (!album.exists || album.data()?.isDeleted === 1)
        throw new ApiError(400, "Choose an active album.");
    }
    ...
    if (buffer.byteLength === 0 || buffer.byteLength > 8 * 1024 * 1024) {
      throw new ApiError(
        413,
        "Each image must be 8 MB or smaller after compression.",
      );
    }
    ...
    if (!validation.valid) {
      throw new ApiError(
        400,
        validation.error || "File did not pass magic bytes verification.",
      );
    }
    ...
    if (!existingPhotoSnap.empty) {
      ...
      if (existingPhotoData.isDeleted === 1) {
        throw new ApiError(
          409,
          "This image is already in the archive. Restore it before uploading it again.",
        );
      }
      ...
      if (albumId && existingPhotoData.albumId !== albumId) {
        const previousAlbumId =
          typeof existingPhotoData.albumId === "string"
            ? existingPhotoData.albumId
            : null;
        ...
        const newAlbumPhotoRef = albumRef
          .collection("photos")
          .doc(existingPhotoDoc.id);
        ...
        if (previousAlbumId) {
          ...
          batch.delete(
            oldAlbumRef.collection("photos").doc(existingPhotoDoc.id),
          );
        }
    ...
    try {
      ...
      logger.warn(
        "photos",
        "A validated upload could not be decoded for responsive image generation",
        error,
      );
