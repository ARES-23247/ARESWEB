...
async function listDtos(
  ...
  return snapshot.docs
    ...
    .filter(
      ({ data }) =>
        includeArchived ||
        (data.isDeleted !== 1 && data.status === "published"),
    )
    .map(({ id, data }) => ({
      ...toDto(id, data),
...
seasonsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!challengeName)
      throw new ApiError(400, "A challenge name is required.");
    ...
    if (!SAFE_DOC_ID.test(seasonId)) {
      throw new ApiError(
        400,
        "Season id may only contain letters, numbers, dashes, and underscores.",
      );
    }
    ...
    res.json({
      success: true,
      id: seasonId,
      season: seasonDto(seasonId, record),
    });
  }),
);
...
awardsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const awardId =
      typeof input.id === "string" && input.id.trim()
        ? input.id.trim()
        : `award_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
    if (!SAFE_DOC_ID.test(awardId)) {
      throw new ApiError(
        400,
        "Award id may only contain letters, numbers, dashes, and underscores.",
      );
    }
