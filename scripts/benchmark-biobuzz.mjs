...
for(const count of [1,5,10,25]){
  ...
  const row={matches:count,elapsedSeconds:(performance.now()-start)/1000,cpuPercentOfOneCore:cpuPercent,p99BatchMs:batches[Math.floor(batches.length*0.99)],maxBatchMs:batches.at(-1),rawMiB:rawBytes/1048576,compressedMiB:compressedBytes/1048576,compressedMiBPerMatch:compressedBytes/count/1048576,rssMiB:process.memoryUsage().rss/1048576,results:worlds.map(w=>({phase:w.phase,tips:w.hives.map(h=>h.tips)}))};
  records.push(row);console.log(JSON.stringify(row));
}
mkdirSync("build/biobuzz",{recursive:true});
writeFileSync("build/biobuzz/benchmark.json",JSON.stringify({machine:process.platform,node:process.version,date:new Date().toISOString(),note:"Local CPU benchmark; repeat on the intended Cloud Run CPU before raising production admission.",records},null,2));
