...
describe("workspace game deployment inputs", () => {
  it("stages canonical rules and identical assets, and refreshes changed source", () => {
    ...
    for (const file of ["game-common/src/hexGrid.ts", "buzzello/src/rules.ts", "buzzle/src/rules.ts",
      "waggle-way/src/core/level.ts", "waggle-way/src/core/weather.ts", "waggle-way/src/core/engine.ts", "waggle-way/src/core/replay.ts", "waggle-way/src/core/community.ts",
      ...["field","hive","shooting","robot","types","timing","scoring","auto","bots","engine","protocol"].map(name=>`biobuzz/src/core/${name}.ts`),
    ...
    for (const name of ["level", "weather", "engine", "replay", "community"]) {
      expect(readFileSync(resolve(root, `functions/src/generated/games/waggle-way/${name}.ts`), "utf8"))
        .toBe("// Generated by games:prepare; edit the canonical package source.\n" + readFileSync(resolve(root, `packages/waggle-way/src/core/${name}.ts`), "utf8"));
    }
    for (const name of ["field","hive","shooting","robot","types","timing","scoring","auto","bots","engine","protocol"]) {
      expect(readFileSync(resolve(root, `functions/src/generated/games/biobuzz/${name}.ts`), "utf8"))
        .toBe("// Generated by games:prepare; edit the canonical package source.\n" + readFileSync(resolve(root, `packages/biobuzz/src/core/${name}.ts`), "utf8"));
    }
