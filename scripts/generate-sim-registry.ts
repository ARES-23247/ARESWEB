 ...
 *    /** @sim {"name": "Display Name", "requiresContext": false, "academyApproved": false} *\/
 * 3. Run: pnpm run generate:sims
 */
...
// Format: /** @sim {"name": "Display Name", "requiresContext": false, "academyApproved": false} */
function parseSimMetadata(filePath: string, folderName: string): SimMetadata {
  ...
  let academyApproved = false;
  let fidelity: SimMetadata['fidelity'] = null;
  ...
  if (simCommentMatch) {
    try {
      ...
      academyApproved = metadata.academyApproved === true;
      fidelity = metadata.fidelity === 'conceptual' || metadata.fidelity === 'code-derived'
        ? metadata.fidelity
        : null;
    } catch {
  ...
  if (requiresContext) academyApproved = false;
  ...
  return { id, name, folder: folderName, requiresContext, academyApproved, fidelity };
}
...
function generateRegistry(sims: SimMetadata[]): string {
  ...
  const academyMappings = sims.filter((sim) => sim.academyApproved).map(sim => {
    const lines = [`  ${sim.id}: ${sim.id},`];
    if (sim.id !== sim.id.toLowerCase()) {
      lines.push(`  ${sim.id.toLowerCase()}: ${sim.id},`);
    }
...
const SIM_METADATA = ${JSON.stringify(sims, null, 2).replace(/"/g, "'")} as const;
