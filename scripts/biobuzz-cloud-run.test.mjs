...
vi.mock("node:fs",async importOriginal=>{const original=await importOriginal();const readFileSync=(path,...args)=>{const text=original.readFileSync(path,...args);if(path!=="infra/gcp/biobuzz-service.json"||!state.enabled)return text;const c=JSON.parse(text);return JSON.stringify({...c,productionEnabled:true,launchVerification:{cloudRunGuardrail:true,totalProjectAlerts:true,targetCapacity:true}});};return {...original,readFileSync,default:{...original.default,readFileSync}};});
vi.mock("node:child_process",async importOriginal=>{const original=await importOriginal();const spawnSync=vi.fn((_command,args)=>{
 ...
 else if(joined.startsWith("auth print-identity-token"))data="signed-test-token";
...
function revision(){return {spec:{template:{metadata:{annotations:{"autoscaling.knative.dev/maxScale":"1","run.googleapis.com/cpu-throttling":"false","run.googleapis.com/startup-cpu-boost":"false","run.googleapis.com/execution-environment":"gen2"}},spec:{serviceAccountName:contract.runtimeServiceAccount,containerConcurrency:128,timeoutSeconds:600,containers:[{image,resources:{limits:{cpu:"1",memory:"1Gi"}},env:[{name:"NODE_ENV",value:"production"},{name:"ENFORCE_APP_CHECK",value:"true"},{name:"BIOBUZZ_MAX_ROOMS",value:"5"},{name:"BIOBUZZ_PUBLIC_ORIGIN",value:contract.publicOrigin},{name:"ABUSE_HMAC_SECRET",valueFrom:{secretKeyRef:{name:"ABUSE_HMAC_SECRET",key:"latest"}}}]}]}}},status:{latestReadyRevisionName:"rev1",latestCreatedRevisionName:"rev1",traffic:[{revisionName:"rev1",percent:100}]}};}
beforeEach(()=>{
...
describe("BIOBUZZ deployment bounds",()=>{
 ...
 it("requires launch evidence and rejects identity, quota, budget, and resource drift",()=>{
  for(const change of [{schemaVersion:2},{runtime:{...contract.runtime,maxInstances:2}},{runtimeServiceAccount:"default"},{secrets:[]},{publicOrigin:"https://evil.example"},{productionEnabled:true}])expect(()=>validateBiobuzzContract({...contract,...change})).toThrow();
 ...
 it("drains, deploys, verifies exact resources and permissions, then reopens admission",async()=>{
  ...
  expect(requests.every(([,init])=>init.headers.Authorization==="Bearer signed-test-token")).toBe(true);
