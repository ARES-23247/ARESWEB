...
- [x] **Phase 67: ESLint Configuration Fix** — Resolve `@typescript-eslint/parser` error and deprecated `.eslintignore` warning by migrating ignore patterns into `eslint.config.js`.
...
- [x] **Phase 63: Sim Layout Refactor** (SIM-09) — Move the simulation live window below the code and AI chat windows to provide more horizontal space for coding. Confirm editor rendering functionality.
- [x] **Phase 64: Fix Sim** — Fix the sim components (specifically addressing PhysicsWorld undefined error in live preview by bundling ares-physics minified file).
