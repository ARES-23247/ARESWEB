...
import { ClipboardList, Plus, Trash2, Shield, Settings, Info, CheckSquare, Layers } from "lucide-react";
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-12 md:py-20">
        ...
        <header className="mb-12 border-b border-ares-bronze/30 pb-8 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
          <div>
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tighter flex flex-wrap items-center gap-3">
              ARES <span className="bg-ares-red px-6 py-2 ares-cut shadow-xl text-white font-bold">Task Board</span>
              {isLive ? (
          ...
          {/* Subteam Filters */}
          <div className="flex flex-wrap gap-2 bg-black/40 p-1.5 rounded-lg border border-white/10 shrink-0">
            {["all", "software", "hardware", "business", "outreach"].map((st) => (
              <button
                key={st}
                onClick={() => setFilterSubteam(st)}
                className={`px-3.5 py-2 text-xs font-bold uppercase rounded transition-all duration-200 ${
                  filterSubteam === st
        ...
        {canEdit ? (
          <form onSubmit={handleCreateTask} className="glass-card hero-card text-marble mb-12 max-w-3xl mx-auto p-8 space-y-6 border border-white/10">
            <h3 className="text-xl font-bold border-b border-ares-bronze/25 pb-3 text-ares-gold flex items-center gap-2 font-heading">
              <Plus size={18} /> Create New Task Card
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider mb-2 text-marble/85">Title</label>
                <input
                  type="text"
                  placeholder="e.g. Calibrate pinpoint pod parameters"
                  value={newTaskTitle}
                  onChange={(e) => setNewTaskTitle(e.target.value)}
                  className="w-full bg-black/60 border border-white/20 rounded px-4 py-2.5 text-sm text-white focus:outline-none focus:border-ares-red transition-colors"
                  required
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider mb-2 text-marble/85">Subteam</label>
                  <select
                    value={newTaskSubteam}
                    onChange={(e) => setNewTaskSubteam(e.target.value as any)}
                    className="w-full bg-black/60 border border-white/20 rounded px-3 py-2.5 text-sm text-white focus:outline-none focus:border-ares-red transition-colors"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider mb-2 text-marble/85">Priority</label>
                  <select
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-2 text-marble/85">Description</label>
              <textarea
                placeholder="Detail technical calibration thresholds, subsystem specs, etc..."
                value={newTaskDesc}
                onChange={(e) => setNewTaskDesc(e.target.value)}
                className="w-full bg-black/60 border border-white/20 rounded px-4 py-2.5 text-sm text-white focus:outline-none focus:border-ares-red h-24 transition-colors"
              />
            </div>
            <div className="flex justify-end">
              <button
                type="submit"
                className="clipped-button-sm bg-ares-red text-white hover:bg-ares-red-dark transition-all"
              >
                Add Task Card
              </button>
            </div>
          ...
          <div className="glass-card ares-cut border border-ares-bronze/20 text-marble/80 px-6 py-5 rounded-xl text-center text-sm font-semibold max-w-lg mx-auto mb-12 flex items-center gap-3 justify-center">
            <Shield size={18} className="text-ares-gold shrink-0" />
            <span>🔒 Guest View Mode: Please sign in above to create, drag, or delete task cards.</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          {columns.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className="glass-card p-5 rounded-2xl border border-white/10 min-h-[600px] flex flex-col"
              >
                <h3 className="font-extrabold text-lg text-white border-b border-white/10 pb-4 mb-4 flex items-center justify-between font-heading">
                  <span className="flex items-center gap-2">
                    <span className="text-xl">{col.emoji}</span> {col.title}
                  </span>
                  <span className="bg-ares-red/20 border border-ares-red/35 text-white text-xs font-bold px-2.5 py-0.5 rounded-full">
                    {colTasks.length}
                  </span>
                </h3>
                ...
                <div className="space-y-4 flex-1">
                  {colTasks.map((task) => {
                    ...
                    return (
                      <div
                        key={task.id}
                        className="bg-black/35 border border-white/15 rounded-xl p-4.5 transition-all duration-200 hover:border-ares-red hover:-translate-y-0.5 shadow-sm flex flex-col justify-between gap-4"
                      >
                        <div>
                          {/* Card tags */}
                          <div className="flex justify-between items-center mb-3">
                            <span className="text-[10px] bg-white/5 border border-white/10 px-2 py-0.5 rounded font-bold uppercase tracking-wider text-marble/80">
                              {task.subteam}
                            </span>
                            <span
                          ...
                          <h4 className="font-bold text-white leading-snug mb-2 font-heading">
                            {task.title}
                          </h4>
                          <p className="text-marble/70 text-xs leading-relaxed mb-4">{task.description}</p>

                          {/* Subtasks checklists */}
                          {totalSub > 0 && (
                            <div className="mb-2 bg-black/40 p-3 rounded-lg border border-white/5">
                              <div className="flex justify-between items-center text-[10px] font-bold text-marble/60 mb-2">
                              ...
                              <div className="space-y-2">
                                {task.subtasks.map((sub) => (
                                  <label
                                    key={sub.id}
                                    className="flex items-center gap-2.5 text-xs text-marble/80 cursor-pointer select-none hover:text-white"
                                  >
                                    <input
                                      type="checkbox"
                                      checked={sub.done}
                                      ...
                                      className="rounded bg-black border-white/20 text-ares-red focus:ring-0 focus:ring-offset-0 disabled:opacity-50"
                                    />
                        ...
                        {/* Controls */}
                        <div className="border-t border-white/10 pt-3 flex justify-between items-center gap-2 mt-auto">
                          {canEdit ? (
                            <>
                              <select
                                value={task.status}
                                onChange={(e) => handleMoveStatus(task.id, e.target.value as any)}
                                className="bg-black/60 border border-white/20 text-marble/90 text-xs font-bold rounded px-2.5 py-1.5 focus:outline-none focus:border-ares-red"
                              >
                              ...
                              <button
                                ...
                                className="text-marble/40 hover:text-ares-danger transition-colors p-1"
                            ...
                            <span className="text-[10px] text-marble/45 italic flex items-center gap-1">
