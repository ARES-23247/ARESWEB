...
router.get("/telemetry-log", auth_1.ensureAuth, async (req, res) => {
    try {
...
router.post("/match-analysis", auth_1.ensureAdmin, async (req, res) => {
    try {
...
router.post("/onshape-sync", auth_1.ensureAdmin, async (req, res) => {
    try {
        const { documentId, workspaceId, elementId, type = "robot", robotId } = req.body;
        ...
        (async () => {
            try {
                ...
                try {
                    ...
                    if (type === "field") {
                        const fieldRef = firebase_admin_1.adminDb.collection("settings").doc("field_cad");
                        await fieldRef.set({
                            documentId,
                            workspaceId,
                            elementId,
                    ...
                    if (type === "robot" && robotId) {
                        const robotRef = firebase_admin_1.adminDb.collection("robots").doc(robotId);
                        await robotRef.set({
