 ...
 * Small sequential batches retain network settling between navigations without
 * charging every route against one cumulative test deadline.
 */
const publicRoutes: Array<{ path: string; titlePart: string }> = [
  ...
  { path: "/buzzhex", titlePart: "BUZZHEX" },
  { path: "/arcade", titlePart: "ARES Arcade" },
...
for (let offset = 0; offset < publicRoutes.length; offset += 5) {
  const batch = publicRoutes.slice(offset, offset + 5);
  test(`public routes ${offset + 1}-${offset + batch.length} render with their titles and a main landmark`, async ({
    page,
    context,
  }) => {
    ...
    for (const { path, titlePart } of batch) {
...
test("same-origin gallery media renders without direct Storage traffic", async ({
  page,
  context,
}) => {
  ...
  await context.route(
    /https:\/\/(?:firebasestorage\.googleapis\.com|storage\.googleapis\.com)\//u,
    async (route) => {
  ...
  const thumbnail = page
    .getByRole("img", { name: "Drive practice progress" })
    .first();
