...
`catalog.json` is the canonical import manifest. Every entry is a draft until a human reviewer approves it in the protected editorial workflow. `source-authorities.json` separately records the current and reviewed historical source identities. Source links are pinned to a released tag or an exact clean repository commit and include the corresponding Git blob hash.

Run `pnpm run content:validate` to validate metadata, approved historical/current commit identities, unique path order, and Markdown files without network access. Run `pnpm run content:verify` to download the exact public source files, independently recompute their Git blob hashes, and verify that the declared current ARESLib tag and commit still match the authoritative repository; CI uses this stronger check. Run `pnpm run content:prepare` to create a Firestore-ready JSON artifact under `build/`; that command does not connect to Firebase or change production data.
