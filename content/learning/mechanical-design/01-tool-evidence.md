...
```mermaid
%% aria: A bounded task and exact workpiece lead to an available exact tool, current approved instructions, workholding and clear-area plan, training and protective requirements, isolation and stop conditions, then a post-task inspection. Missing evidence returns the plan to review before work starts.
flowchart LR
  T["bounded task"] --> W["exact workpiece"]
  W --> E["exact available tool"]
  E --> I["current approved instructions"]
  I --> H["workholding and clear area"]
  H --> P["training and protection"]
  P --> S["isolation and stop conditions"]
  S --> C["post-task check"]
```
