  ...
  return (
    <div className="flex-grow flex flex-col justify-between overflow-y-auto space-y-6 scrollbar-thin scrollbar-thumb-white/5 text-left">
      <div className="space-y-4">
        ...
        {uploadError && (
          <p role="alert" className="text-[9px] font-mono text-white bg-ares-red/15 p-2 rounded border border-ares-red/40 max-w-md">
            {uploadError}
          </p>
        )}
        ...
        {pendingPhoto && (
          <div role="alertdialog" aria-label="Confirm event photo archive" className="rounded border border-ares-red/35 bg-ares-red/10 p-4 text-sm text-white">
            <p>Archive <strong>{pendingPhoto.filename || "this photo"}</strong> from the event gallery?</p>
            <p className="mt-1 text-xs text-marble/70">This keeps the record available for recovery.</p>
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={() => setPendingPhotoId(null)} className="rounded border border-white/15 px-3 py-2 text-xs font-bold focus-visible:ring-2 focus-visible:ring-ares-cyan">Cancel</button>
              <button type="button" autoFocus onClick={() => { handleDeletePhoto(pendingPhoto.id); setPendingPhotoId(null); }} className="rounded bg-ares-red px-3 py-2 text-xs font-bold text-white focus-visible:ring-2 focus-visible:ring-ares-cyan">Archive photo</button>
            </div>
          </div>
        )}
      </div>
