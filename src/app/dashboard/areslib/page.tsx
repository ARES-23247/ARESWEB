...
export default function AreslibManagementPage() {
  ...
  const [formCategory, setFormCategory] = useState("Core Math & Control");
  ...
  // Display destination flags - default ARESLib to true for ARESLib Manager
  const [formDisplayInAreslib, setFormDisplayInAreslib] = useState(true);
  ...
  // 1. Listen for real-time docs updates (Filtered for ARESLib context)
  useEffect(() => {
    try {
      ...
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          ...
          const list = snapshot.docs
            ...
            .filter((d) => d.isDeleted !== 1 && d.displayInAreslib === 1);
  ...
  const handleOpenCreate = () => {
    ...
    setFormTitle("");
    setFormSlug("");
    setFormCategory("Core Math & Control");
  ...
  const handleOpenEdit = (docItem: DocRecord) => {
    ...
    if (ARESLIB_CATEGORIES.includes(docItem.category)) {
  ...
  const handleRevertToRevision = (rev: DocRevision) => {
    ...
    if (ARESLIB_CATEGORIES.includes(rev.category)) {
    ...
    <div className="space-y-10 w-full text-left">
      ...
      <header className="border-b border-white/5 pb-8 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div>
          <p className="text-ares-gold font-bold uppercase tracking-widest text-xs mb-3 font-heading flex items-center gap-2">
            <Activity size={12} className="animate-pulse" /> Codebase Library
          </p>
      ...
      <div className="glass-card border border-white/10 ares-cut-lg overflow-hidden shadow-xl">
        <div className="p-6 border-b border-white/5 bg-black/20 flex justify-between items-center text-xs font-black uppercase text-ares-gold tracking-widest">
          <span>Active ARESLib Documentation</span>
          <span>{loadingList ? "Loading..." : `${docs.length} Documents`}</span>
        </div>
              ...
              {loadingList ? (
                <tr>
                  <td colSpan={6} className="p-12 text-center text-marble/40 font-mono">
                    Syncing documentation indices...
                  </td>
                </tr>
              ) : docs.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-12 text-center text-marble/40 font-mono">
                    No articles indexed. Create a new document to get started.
                  </td>
                </tr>
              ) : (
                docs.map((docItem) => {
          ...
          >
            <header className="px-6 py-4.5 border-b border-white/10 flex items-center justify-between bg-black/20 shrink-0">
              <div>
                <h3 className="text-white font-extrabold text-lg font-heading uppercase tracking-tight">
                  {editSlug ? `Edit Document: ${formTitle}` : "Create ARESLib Document"}
                </h3>
                  ...
                  >
                    <div className="space-y-6 pb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          <label htmlFor="formCategory" className="block text-[10px] font-bold uppercase tracking-wider mb-2 text-marble/60">Architecture layer</label>
                          <select
                            id="formCategory"
                            value={formCategory}
                            onChange={(e) => {
                      ...
                      <div>
                        ...
                        <textarea
                          id="formDescription"
                          rows={2}
                          placeholder="A quick overview sentence summarizing the article or document contents."
                      ...
                      <div>
                        <label htmlFor="formContent" className="block text-[10px] font-bold uppercase tracking-wider mb-2 text-marble/60">Document Content (Markdown & LaTeX)</label>
                        <MarkdownEditor
                          id="formContent"
                            ...
                            {suggestedCorrection && grammarEdits.length > 0 && (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setGrammarEdits([]);
                                  setSuggestedCorrection("");
                                  setRevertAlert("Applied grammar and spelling corrections to the document draft!");
                                }}
            ...
            {activeTab === "edit" && (
              <footer className="px-6 py-4 border-t border-white/10 flex justify-end gap-3 bg-black/20 shrink-0">
                ...
                >
                  {editSlug ? "Update Entry" : "Publish Document"}
                </button>
              </footer>
            )}
          </div>
