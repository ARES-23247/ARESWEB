...
export default function TournamentDetailPage() {
  ...
  const [activeLightboxImage, setActiveLightboxImage] = useState<TournamentPhoto | null>(null);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await authenticatedFetch(`/api/photos?albumId=${encodeURIComponent(tournament.photoAlbumId)}&limit=50`);
  ...
  const handleMutationSuccess = () => {
    ...
    void queryClient.invalidateQueries({ queryKey: ["tournament_matches", id] });
  };
  ...
  const updateMatchMutation = useMutation({
    mutationFn: async (updated: Partial<TournamentMatch> & { id: string }) => {
      ...
      return updateTournamentMatch(id, updated.id, {
        ...
        scoreSelf: updated.scoreSelf ?? undefined,
        scoreOpponent: updated.scoreOpponent ?? undefined,
  ...
  if (authLoading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[60vh] bg-obsidian text-marble">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-ares-gold/85 animate-pulse font-heading">Connecting Data Nodes...</p>
      </div>
    );
  }
  ...
  if (!isAuthorized) {
    return (
      <div className="w-full min-h-screen bg-obsidian text-marble py-8 flex flex-col justify-center relative overflow-hidden">
        <SEO title="Sign In | ARES Scouting Vault" description="Authorized authentication required to access team scouting records." noindex={true} />
        <div className="absolute top-0 left-0 w-full z-10"><GreekMeander variant="thin" opacity="opacity-30" className="w-full" /></div>
        <div className="w-full max-w-md mx-auto px-6 z-10">
          <div className="glass-card hero-card p-8 border border-white/10 bg-black/60 shadow-2xl flex flex-col items-center text-center">
            <AlertCircle className="text-ares-gold w-12 h-12 mb-4 animate-bounce" aria-hidden="true" />
            <h2 className="text-xl font-bold uppercase tracking-tight text-white mb-2">Access Gated</h2>
            <p className="text-xs text-marble/60 mb-6">You must log in to view tournament matches, scouting reports, and OPR logs.</p>
            <Link to="/tournaments" className="clipped-button bg-ares-red text-white uppercase text-xs w-full py-2">Back to Tournaments</Link>
          </div>
        </div>
      </div>
    );
  ...
  if (isTournamentError) {
    const isNotFound = tournamentError instanceof TournamentApiError && tournamentError.status === 404;
    return (
        ...
        <div className="mx-auto max-w-3xl">
          {isNotFound ? (
            <div className="text-center p-8 hero-card bg-white/5 border border-white/10">
              <Trophy size={48} className="mx-auto text-ares-gold mb-4" aria-hidden="true" />
              <h1 className="text-xl font-bold text-white uppercase mb-2">Record Not Found</h1>
              <p className="text-xs text-marble/60 mb-6">The tournament record may have been archived or the link may be out of date.</p>
              <Link to="/tournaments" className="clipped-button bg-ares-red text-white uppercase text-xs py-2 px-6">Back to List</Link>
            </div>
          ) : (
            <PublicDataState title="Unable to load this tournament" message="The scouting record could not be reached. Check your connection or sign in again, then retry." diagnostic={tournamentError instanceof Error ? tournamentError.message : String(tournamentError)} onRetry={() => void refetchTournament()} />
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full min-h-screen bg-obsidian text-marble py-8">
      <SEO title={`${tournament.name} | Scouting & Analytics`} description={`Match results, OPR values, and technical scouting logs for ARES at the ${tournament.name}.`} noindex={true} />
          ...
          <div className="lg:col-span-2 space-y-8">
            {isMatchesError && (
              <PublicDataState
                ...
                message={matches.length > 0 ? "The last confirmed match list is still shown below, but its refresh failed." : "Tournament details are available, but the match checklist could not be reached."}
                diagnostic={matchesError instanceof Error ? matchesError.message : String(matchesError)}
                onRetry={() => void refetchMatches()}
              />
            )}
            {mutationError && (
              <div role="status" className="rounded-xl border border-ares-red/40 bg-ares-red/10 p-4 text-sm text-white">
                <p className="font-bold">{mutationError instanceof TournamentApiError && mutationError.status === 404 ? "That match was changed or archived elsewhere. The list has been refreshed." : "The match change was not saved. Your confirmed data is unchanged."}</p>
                <p className="mt-1 font-mono text-xs text-marble/80">{mutationError.message}</p>
                <button type="button" onClick={() => setMutationError(null)} className="mt-3 rounded border border-white/20 px-3 py-1 text-xs font-bold uppercase focus-visible:ring-2 focus-visible:ring-ares-cyan">Dismiss</button>
              </div>
            )}
            ...
            <TournamentPhotosSection hasAlbum={Boolean(tournament.photoAlbumId)} photos={photos} isError={isPhotosError} error={photosError} onRetry={() => void refetchPhotos()} onOpenPhoto={setActiveLightboxImage} />
          </div>
      ...
      <TournamentPhotoLightbox photo={activeLightboxImage} onClose={() => setActiveLightboxImage(null)} />
    </div>
  );
}
