...
export default function VideosPage() {
  ...
  const loadVideos = useCallback(
    async (append = false, targetCursor: string | null = null) => {
      ...
      try {
        ...
        if (!response.ok)
          throw await apiFailure(response, "Published videos could not load.");
        const payload = await response.json().catch(() => {
          throw new Error(
            `HTTP ${response.status} ${response.statusText || "OK"}: The video API returned invalid JSON.`,
          );
        });
  ...
  const selectAdjacentVideo = useCallback(
    (direction: -1 | 1) => {
      ...
      const currentIndex = visibleVideos.findIndex(
        (video) => video.id === selected.id,
      );
      const nextIndex =
        (Math.max(currentIndex, 0) + direction + visibleVideos.length) %
  ...
  return (
    <div className="min-h-screen bg-obsidian text-marble">
      <SEO
        title="Video Hub"
        description="Watch ARES 23247 match recordings, robot reveals, team highlights, and learning guides."
      />
      <section className="relative flex min-h-[45vh] items-center overflow-hidden py-24">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 text-center">
          <p className="mb-4 text-[10px] font-black uppercase tracking-[0.35em] text-ares-bronze">
            Team highlights and learning
          </p>
      ...
      <main
        className="border-y border-white/10 bg-black/10 py-12"
        id="main-content"
      >
        <div className="mx-auto max-w-7xl px-6">
          ...
          {error && (
            <div
              role="alert"
              className="mb-8 border border-ares-red bg-ares-red/15 p-4 text-white"
            >
          ...
          {loading && videos.length === 0 ? (
            <div
              role="status"
              className="flex justify-center py-24 text-ares-gold"
            >
              <Loader2
          ...
          ) : error && videos.length === 0 ? null : visibleVideos.length ===
            0 ? (
            <div className="border border-white/10 p-16 text-center">
              <Film
                className="mx-auto mb-3 text-marble/30"
                aria-hidden="true"
              />
              ...
              {visibleVideos.map((video) => (
                ...
                >
                  ...
                  <div className="space-y-3 p-5">
                    ...
                    {video.description && (
                      <p className="line-clamp-3 text-sm leading-relaxed text-marble/70">
                        {video.description}
                      </p>
                    )}
                    <a
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-modal max-h-[95vh] w-[calc(100%-2rem)] max-w-5xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto border border-white/15 bg-obsidian focus:outline-none">
            <div className="flex items-start justify-between gap-4 border-b border-white/10 p-4">
              <div>
                <div className="flex items-center gap-2.5">
                  ...
                  {visibleVideos.length > 1 && selected && (
                    <span className="text-[10px] font-bold uppercase tracking-wider text-marble/60">
                      {visibleVideos.findIndex((v) => v.id === selected.id) + 1}{" "}
                      of {visibleVideos.length}
                    </span>
                  )}
                </div>
                ...
                <Dialog.Description className="mt-1 text-xs text-marble/60">
                  Team YouTube video player
                </Dialog.Description>
              </div>
