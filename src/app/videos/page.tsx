...
export default function VideosPage() {
  ...
  return <div className="min-h-screen bg-obsidian text-marble">
    ...
    <section className="relative flex min-h-[45vh] items-center overflow-hidden py-24">
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6 text-center">
        ...
        <h1 className="font-heading text-5xl font-black uppercase tracking-tight text-white md:text-7xl">Video <span className="bg-ares-red px-4 py-1 text-white">Hub</span></h1>
        <p className="mx-auto mt-6 max-w-2xl border-t border-white/10 pt-6 text-base leading-relaxed text-marble/85">Watch the robot compete, see how we build, and learn with Team ARES.</p>
      </div>
    </section>
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-10 flex flex-col gap-5 border-b border-white/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div role="group" aria-label="Filter videos" className="flex flex-wrap gap-2">
            {([['all', 'All media'], ['video', 'Videos'], ['short', 'Shorts']] as const).map(([value, label]) => <button key={value} type="button" aria-pressed={filter === value} onClick={() => setFilter(value)} className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider focus-visible:ring-2 focus-visible:ring-ares-cyan ${filter === value ? "bg-ares-red text-white" : "border border-white/10 bg-white/5 text-marble/70"}`}>{label}</button>)}
          </div>
          <a href="https://www.youtube.com/@ares23247WV" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 border border-white/15 px-4 py-2 text-[10px] font-black uppercase tracking-wider text-white focus-visible:ring-2 focus-visible:ring-ares-cyan">Team YouTube <ExternalLink size={12} aria-hidden="true" /></a>
        </div>
        ...
        {loading && videos.length === 0 ? <div role="status" className="flex justify-center py-24 text-ares-gold"><Loader2 className="motion-safe:animate-spin" aria-hidden="true" /><span className="sr-only">Loading videos</span></div> : error && videos.length === 0 ? null : visibleVideos.length === 0 ? <div className="border border-white/10 p-16 text-center"><Film className="mx-auto mb-3 text-marble/30" aria-hidden="true" /><p className="text-sm text-marble/60">No videos match this filter.</p></div> : <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {visibleVideos.map((video) => <article key={video.id} className="overflow-hidden border border-white/10 bg-black/30">
            <button type="button" onClick={() => setSelected(video)} className="group relative block aspect-video w-full overflow-hidden bg-black focus-visible:ring-2 focus-visible:ring-ares-cyan" aria-label={`Play ${video.title}`}>
              ...
              <span className="absolute inset-0 flex items-center justify-center bg-black/25"><span className="rounded-full bg-ares-red p-4 text-white"><Play size={20} fill="currentColor" aria-hidden="true" /></span></span>
              <span className="absolute left-3 top-3 bg-ares-red px-2 py-1 text-[9px] font-black uppercase text-white">{video.type}</span>
            </button>
    ...
    <Dialog.Root open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-modal bg-black/90" /><Dialog.Content className="fixed left-1/2 top-1/2 z-modal max-h-[95vh] w-[calc(100%-2rem)] max-w-5xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto border border-white/15 bg-obsidian focus:outline-none"><div className="flex items-start justify-between gap-4 border-b border-white/10 p-4"><div><Dialog.Title className="font-heading text-lg font-black uppercase text-white">{selected?.title}</Dialog.Title><Dialog.Description className="mt-1 text-xs text-marble/60">Team YouTube video player</Dialog.Description></div><Dialog.Close asChild><button type="button" aria-label="Close video" className="p-2 text-white focus-visible:ring-2 focus-visible:ring-ares-cyan"><X aria-hidden="true" /></button></Dialog.Close></div>{selected && <><div className="aspect-video bg-black"><iframe src={`${selected.embedUrl}?autoplay=1&rel=0`} title={selected.title} loading="lazy" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen className="h-full w-full" /></div>{selected.description && <p className="p-5 text-sm leading-relaxed text-marble/70">{selected.description}</p>}</>}</Dialog.Content></Dialog.Portal></Dialog.Root>
  </div>;
}
