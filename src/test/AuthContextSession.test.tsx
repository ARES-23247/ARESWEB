...
function SessionProbe() {
  ...
  return (
    <output aria-label="session state">
      {loading ? "loading" : `${user?.uid || "signed-out"}:${authorizedUser?.role || "no-role"}`}
    </output>
  );
}
...
describe("AuthProvider backend session linking", () => {
  beforeEach(() => {
    ...
    mocks.onAuthStateChanged.mockImplementation((_auth, callback: (user: typeof mocks.currentUser) => void) => {
  ...
  it("links an authenticated Firebase user through the verified session API", async () => {
    ...
    expect(await screen.findByText("firebase-user-1:admin")).toBeInTheDocument();
    expect(mocks.authenticatedFetch).toHaveBeenCalledWith("/api/profiles/session", {
      method: "POST",
  ...
  it("keeps the user signed in but grants no role when session linking fails", async () => {
    ...
    expect(await screen.findByText("firebase-user-1:no-role")).toBeInTheDocument();
  });
});
