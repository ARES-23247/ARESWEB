...
export default function PlatformQuickStats({ stats }: PlatformQuickStatsProps) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-bold text-white text-sm uppercase tracking-widest flex items-center gap-2 mb-2 ml-1">
      ...
      <Grid numItems={2} numItemsSm={2} numItemsLg={4} className="gap-4">
        {data.map(stat => (
          <Card key={stat.label} className="bg-ares-gray-dark/30 border-white/5 ares-cut-sm p-4 text-center" decoration="top" decorationColor={stat.color}>
            <Text className="text-marble/80 uppercase tracking-widest font-bold text-[10px] mb-1">{stat.label}</Text>
            <Metric className="text-white font-bold text-2xl">{stat.value}</Metric>
          </Card>
        ))}
      </Grid>
    </div>
