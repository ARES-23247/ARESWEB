...
export default class ErrorBoundary extends Component<Props, State> {
  ...
  public render() {
    if (this.state.hasError) {
      ...
      return (
        <div role="alert" className="flex flex-col items-center justify-center min-h-[400px] w-full bg-obsidian text-white ares-cut-sm border border-dashed border-ares-red/50 p-8 shadow-2xl">
          <div className="mb-6 rounded bg-ares-red p-3 text-white">
            <svg aria-hidden="true" className="w-10 h-10 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h2 className="text-2xl font-bold tracking-tight text-white mb-2">This section could not be displayed</h2>
          <p className="text-white/60 text-center max-w-lg mb-6 leading-relaxed">
            Reload the page to try again. If the problem continues, share the support ID below with an ARES administrator.
          </p>
          ...
          <div className="flex gap-4 mb-4">
            ...
            {this.state.statusCode && (
              <div className="bg-black/40 px-3 py-1.5 ares-cut-sm border border-white/10 flex flex-col items-center">
                ...
                <span className="rounded bg-ares-red px-2 py-0.5 text-sm font-mono text-white">{this.state.statusCode}</span>
              </div>
            )}
          </div>

          {import.meta.env.DEV && (
            <details className="bg-black/40 border border-white/10 p-4 ares-cut-sm w-full max-w-xl text-left text-xs text-white overflow-x-auto mb-8 shadow-inner">
              <summary className="cursor-pointer font-bold text-marble/80 focus-visible:ring-2 focus-visible:ring-ares-cyan">
                Technical details
              </summary>
              <pre className="mt-3 whitespace-pre-wrap break-words font-mono text-marble/70">{this.state.errorStr}</pre>
            </details>
          )}
          <button 
