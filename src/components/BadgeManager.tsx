      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-zinc-800 flex justify-between items-center">
          ...
          <button 
            ...
            className="px-4 py-2 bg-ares-red/10 text-ares-red hover:bg-ares-red hover:text-white transition-all rounded-lg text-sm font-bold flex items-center gap-2"
          >
          ...
          <div className="p-6 bg-black/40 border-b border-zinc-800 space-y-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <input id="badge-id" type="text" value={newBadgeId} onChange={e => setNewBadgeId(e.target.value)} placeholder="e.g. outreach_mvp" className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-ares-gold mt-1" />
              </div>
              <div>
                <label htmlFor="badge-name" className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-1">Display Name</label>
                <input id="badge-name" type="text" value={newBadgeName} onChange={e => setNewBadgeName(e.target.value)} placeholder="Outreach MVP" className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-ares-gold mt-1" />
              </div>
              <div>
                <label htmlFor="badge-icon" className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-1">Icon (Lucide Node)</label>
                <input id="badge-icon" type="text" value={newBadgeIcon} onChange={e => setNewBadgeIcon(e.target.value)} placeholder="Award" className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-ares-gold mt-1" />
              </div>
              <div>
                <label htmlFor="badge-color" className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-1">Color Theme (CSS)</label>
                <input id="badge-color" type="text" value={newBadgeColor} onChange={e => setNewBadgeColor(e.target.value)} placeholder="ares-gold" className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-ares-gold mt-1" />
              </div>
            </div>
            <div>
              <label htmlFor="badge-desc" className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-1">Description</label>
              <input id="badge-desc" type="text" value={newBadgeDesc} onChange={e => setNewBadgeDesc(e.target.value)} placeholder="Awarded to members who attain top 3 in outreach hours." className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-ares-gold mt-1" />
            </div>
            <button 
              onClick={() => createBadgeMutation.mutate()} 
              disabled={createBadgeMutation.isPending || !newBadgeId || !newBadgeName}
              className="px-6 py-2.5 bg-zinc-100 text-black font-bold text-sm rounded-xl hover:bg-white transition-all disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6 bg-zinc-950">
          {badgesLoading ? (
            ...
            badges.map(b => {
              ...
              return (
                <div key={b.id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex items-start gap-4">
                  <div className={`p-3 rounded-lg bg-zinc-800/50 flex-shrink-0 text-${b.color_theme.replace("text-", "")}`}>
                    <IconComp size={24} />
                  </div>
                  <div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            ...
            <select id="grant-user" value={selectedUser} onChange={e => setSelectedUser(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-ares-red mt-1">
          ...
          <div className="flex-1">
            ...
            <select id="grant-badge" value={selectedBadge} onChange={e => setSelectedBadge(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-ares-red mt-1">
        ...
        <button 
          ...
          className="mt-6 px-8 py-3 bg-ares-red text-white font-black uppercase tracking-widest rounded-xl hover:bg-red-600 transition-all disabled:opacity-50 flex items-center gap-2"
        >
