...
import { useGetFinanceSummary, useListFinanceTransactions } from "../api";
...
export default function FinanceLedger() {
  ...
  // ── Database Queries ──
  const { data: summary, isLoading: summaryLoading } = useGetFinanceSummary(selectedSeason);
  const { data: transactionsData, isLoading: transactionsLoading } = useListFinanceTransactions(selectedSeason);
  ...
  const totalExpenseFiltered = Object.values(categorySummary).reduce((a, b) => a + b, 0);
                  ...
                  <div className="flex bg-black/40 p-1 ares-cut-sm border border-white/5 shrink-0">
                    {(["all", "income", "expense"] as const).map((filter) => (
                      <button
                        ...
                        className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all ares-cut-sm ${
