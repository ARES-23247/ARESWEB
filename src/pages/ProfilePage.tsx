    ...
    <div className="min-h-screen bg-obsidian">
      <div className="max-w-3xl mx-auto px-6 py-16">
        <div className="flex justify-between items-center mb-8 print:hidden">
          <Link to="/about" className="inline-flex items-center gap-2 text-marble/60 hover:text-white text-sm font-bold transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="bg-white/5 border border-white/10 ares-cut-lg p-8 flex flex-col md:flex-row items-center md:items-start gap-8 mb-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              {profile.pronouns && <p className="text-marble/60 text-sm mb-3">{profile.pronouns}</p>}
              ...
              {profile.bio && <p className="text-marble/60 text-sm leading-relaxed">{profile.bio}</p>}
            </div>
          </div>
                ...
                {badges.map((b) => {
                  ...
                  return (
                    <div key={b.id} className="relative group cursor-help bg-obsidian border border-white/10 hover:border-ares-gold ares-cut p-4 transition-all flex flex-col items-center justify-center w-28 h-28">
                      ...
                      <span className="text-[10px] font-bold text-center text-marble/60 leading-tight block">{b.name}</span>
                      ...
                      <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 w-48 bg-black border border-white/10 text-white text-xs ares-cut-sm p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-2xl">
                        ...
                        <p className="text-marble/60 text-[10px] leading-relaxed">{b.description}</p>
                        {b.awarded_at && (
                          <p className="text-marble/40 text-[9px] mt-2 border-t border-white/10 pt-2">
             ...
             <div className="bg-ares-red/5 border border-ares-red/20 ares-cut p-6 mb-8 relative overflow-hidden">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                 {(profile.emergency_contact_name || profile.emergency_contact_phone) && (
                   <div>
                     <p className="text-[10px] font-bold text-marble/60 uppercase">Emergency Contact</p>
                     <p className="text-marble/60 text-sm">{profile.emergency_contact_name || "Unknown"}</p>
                     <p className="text-marble/40 text-xs">{profile.emergency_contact_phone}</p>
                   </div>
                 )}
                 {profile.dietary_restrictions && (
                   <div>
                     <p className="text-[10px] font-bold text-marble/60 uppercase">Dietary Info</p>
                     <p className="text-marble/60 text-sm">{JSON.parse(profile.dietary_restrictions || "[]").join(", ")}</p>
                   </div>
                 )}
                 {profile.tshirt_size && (
                   <div>
                     <p className="text-[10px] font-bold text-marble/60 uppercase">T-Shirt Size</p>
                     <p className="text-marble/60 text-sm uppercase">{profile.tshirt_size}</p>
                   </div>
                 )}
               </div>
             </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {profile.rookie_year && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                <p className="text-[10px] font-bold text-marble/60 uppercase tracking-wider mb-2">Rookie Year</p>
                <p className="text-marble/60 text-sm">{profile.rookie_year}</p>
              </div>
            )}
            {profile.leadership_role && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                <p className="text-[10px] font-bold text-ares-red uppercase tracking-wider mb-2">Leadership Role</p>
                <p className="text-marble/60 text-sm">{profile.leadership_role}</p>
              </div>
            )}
            {profile.favorite_first_thing && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                ...
                <p className="text-marble/60 text-sm">{profile.favorite_first_thing}</p>
              </div>
            )}
            {profile.favorite_robot_mechanism && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                ...
                <p className="text-marble/60 text-sm">{profile.favorite_robot_mechanism}</p>
              </div>
            )}
            {profile.pre_match_superstition && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                ...
                <p className="text-marble/60 text-sm">{profile.pre_match_superstition}</p>
              </div>
            )}
            {profile.fun_fact && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                <p className="text-[10px] font-bold text-ares-gold uppercase tracking-wider mb-2">Fun Fact</p>
                <p className="text-marble/60 text-sm">{profile.fun_fact}</p>
              </div>
            )}
            {profile.favorite_food && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                <p className="text-[10px] font-bold text-ares-cyan uppercase tracking-wider mb-2">Favorite Food</p>
                <p className="text-marble/60 text-sm">{profile.favorite_food}</p>
              </div>
            )}
            {profile.grade_year && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                <p className="text-[10px] font-bold text-marble/60 uppercase tracking-wider mb-2">Class</p>
                <p className="text-marble/60 text-sm">{profile.grade_year}</p>
              </div>
            )}
            {(profile.email || profile.phone) && (
              <div className="bg-white/5 border border-white/10 ares-cut p-6">
                <p className="text-[10px] font-bold text-marble/60 uppercase tracking-wider mb-2">Contact</p>
                {profile.email && <p className="text-marble/60 text-sm">{profile.email}</p>}
                {profile.phone && <p className="text-marble/40 text-sm">{profile.phone}</p>}
              </div>
            )}
          </div>
                ...
                {colleges.map((col: { domain: string, name: string, degree: string, years: string }, i: number) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-marble/40 text-xs">{[col.degree, col.years].filter(Boolean).join(" · ")}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {employers.map((emp: { domain: string, name: string, current: boolean, title: string, years: string }, i: number) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-marble/40 text-xs">{[emp.title, emp.years].filter(Boolean).join(" · ")}</p>
                    </div>
                  </div>
                ))}
              </div>
