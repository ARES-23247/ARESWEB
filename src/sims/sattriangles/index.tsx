...
const QUIZ_QUESTIONS: Question[] = [
  {
    ...
    correctIdx: 1,
    explanation:
      "Recall that sin(A) = Opposite / Hypotenuse. Here, the side opposite to angle A is BC, and the hypotenuse is AB = 15. So, sin(A) = BC / AB => 0.6 = BC / 15. Solving for BC gives BC = 15 * 0.6 = 9.",
  ...
  },
  {
    ...
    correctIdx: 2,
    explanation:
      "In a 30°-60°-90° special right triangle, the side lengths are in the ratio x : x√3 : 2x. Since the hypotenuse is 2x = 12, the shorter leg (opposite 30°) is x = 6. The longer leg (opposite 60°) is x√3 = 6√3.",
  ...
  },
  {
    ...
    correctIdx: 1,
    explanation:
      "Recall that tan(A) = Opposite / Adjacent = BC / AC = BC / 8. Since tan(A) = 3/4, we have BC / 8 = 3/4 => BC = 6. This is a classic 6-8-10 right triangle! Using the Pythagorean theorem, the hypotenuse AB = √(6² + 8²) = √(36 + 64) = √100 = 10.",
...
export default function TrianglesSim() {
  ...
  const [specialType, setSpecialType] = useState<"30-60-90" | "45-45-90">(
    "30-60-90",
  );
      ...
      <div className="w-full flex flex-col items-center gap-4 bg-obsidian/40 p-4 rounded-xl border border-white/5 relative">
        ...
        <div className="relative bg-obsidian-darker p-3 rounded-lg border border-white/10 shadow-2xl max-w-full overflow-hidden w-full flex justify-center">
          ...
          >
            ...
            >
              {activeTab === "practice" &&
              QUIZ_QUESTIONS[currentQIdx].setupTriangle.hideA
                ? "a = ?"
            ...
            >
              {activeTab === "practice" &&
              QUIZ_QUESTIONS[currentQIdx].setupTriangle.hideB
                ? "b = ?"
            ...
            >
              {activeTab === "practice" &&
              QUIZ_QUESTIONS[currentQIdx].setupTriangle.hideC
                ? "c = ?"
        ...
        {activeTab !== "special" && (
          <fieldset className="grid w-full grid-cols-1 gap-3 rounded-lg border border-white/10 bg-obsidian-surface/60 p-4 text-xs sm:grid-cols-2">
            <legend className="px-1 font-bold text-ares-gold">
              Triangle side controls
            </legend>
            <label className="flex flex-col gap-1.5 font-bold text-marble">
              Height (opposite side): {(sideA / 10).toFixed(1)}
              <input
                type="range"
                min={30}
                max={220}
                step={1}
                value={sideA}
                onChange={(event) => setSideA(Number(event.target.value))}
                className="w-full accent-ares-red"
              />
            </label>
            <label className="flex flex-col gap-1.5 font-bold text-marble">
              Base (adjacent side): {(sideB / 10).toFixed(1)}
              <input
                type="range"
                min={40}
                max={270}
                step={1}
                value={sideB}
                onChange={(event) => setSideB(Number(event.target.value))}
                className="w-full accent-ares-red"
              />
            </label>
          </fieldset>
        ...
        {activeTab === "sohcahtoa" && (
          ...
          >
            ...
            <div className="bg-obsidian-surface/60 border border-white/5 p-3 rounded-lg flex flex-col items-center">
              <span className="text-xs text-ares-muted font-bold mb-1">
                SINE (SOH)
              </span>
              <span className="text-sm font-heading font-black text-ares-red">
                sin(θ) = Opp / Hyp
              </span>
              <div className="my-1.5 h-px bg-white/5 w-full" />
              <span className="text-xs font-monospace font-semibold text-white/90">
                {(sideA / 10).toFixed(1)} / {(hypotenuse / 10).toFixed(1)} ={" "}
                <span className="text-ares-red">
                  {(sideA / hypotenuse).toFixed(3)}
                </span>
              </span>
            </div>
            ...
            <div className="bg-obsidian-surface/60 border border-white/5 p-3 rounded-lg flex flex-col items-center">
              ...
              <span className="text-sm font-heading font-black text-ares-gold">
                cos(θ) = Adj / Hyp
              </span>
              <div className="my-1.5 h-px bg-white/5 w-full" />
              <span className="text-xs font-monospace font-semibold text-white/90">
                {(sideB / 10).toFixed(1)} / {(hypotenuse / 10).toFixed(1)} ={" "}
                <span className="text-ares-gold">
                  {(sideB / hypotenuse).toFixed(3)}
                </span>
              </span>
            </div>
            ...
            <div className="bg-obsidian-surface/60 border border-white/5 p-3 rounded-lg flex flex-col items-center">
              ...
              <span className="text-sm font-heading font-black text-ares-bronze">
                tan(θ) = Opp / Adj
              </span>
              <div className="my-1.5 h-px bg-white/5 w-full" />
              <span className="text-xs font-monospace font-semibold text-white/90">
                {(sideA / 10).toFixed(1)} / {(sideB / 10).toFixed(1)} ={" "}
                <span className="text-ares-bronze">
                  {(sideA / sideB).toFixed(3)}
                </span>
              </span>
            </div>
          </div>
              ...
              {specialType === "30-60-90" ? (
                <>
                  <p>
                    ...
                    is formed by slicing an equilateral triangle down its
                    center.
                  </p>
                  <ul className="list-disc list-inside flex flex-col gap-1 text-ares-muted">
                    <li>
                      The shorter side (opposite 30°) has length{" "}
                      <strong className="text-ares-red">x = 8.0</strong>.
                    </li>
                    <li>
                      The longer side (opposite 60°) has length{" "}
                      <strong className="text-ares-gold">x√3 = 13.8</strong>{" "}
                      (since 8 * 1.732 ≈ 13.8).
                    </li>
                    <li>
                      The hypotenuse is exactly double the short side:{" "}
                      <strong className="text-ares-bronze">2x = 16.0</strong>.
                    </li>
                  </ul>
                  <p className="border-t border-white/5 pt-2 mt-1 font-semibold text-white/90">
                    Trig Ratios: sin(30°) = 0.500 | cos(30°) = √3/2 ≈ 0.866 |
                    tan(30°) = 1/√3 ≈ 0.577
                  </p>
                </>
              ) : (
                <>
                  <p>
                    ...
                    is an isosceles right triangle formed by cutting a square
                    diagonally.
                  </p>
                  <ul className="list-disc list-inside flex flex-col gap-1 text-ares-muted">
                    <li>
                      The two perpendicular sides are identical:{" "}
                      <strong className="text-ares-red">x = 10.0</strong>.
                    </li>
                    <li>
                      The hypotenuse is the side length times √2:{" "}
                      <strong className="text-ares-bronze">x√2 = 14.1</strong>{" "}
                      (since 10 * 1.414 ≈ 14.1).
                    </li>
                  </ul>
                  <p className="border-t border-white/5 pt-2 mt-1 font-semibold text-white/90">
                    Trig Ratios: sin(45°) = 1/√2 ≈ 0.707 | cos(45°) = 1/√2 ≈
                    0.707 | tan(45°) = 1.000
                  </p>
                </>
              )}
