export const BUZZELLO_BOARD_RADIUS = 4;
...
export const BUZZELLO_DIRECTIONS: ReadonlyArray<BuzzelloCoordinate> = [
  { q: 1, r: 0 },
  { q: 1, r: -1 },
  { q: 0, r: -1 },
  { q: -1, r: 0 },
...
function buildCoordinates(): BuzzelloCoordinate[] {
  ...
  for (let q = -BUZZELLO_BOARD_RADIUS; q <= BUZZELLO_BOARD_RADIUS; q += 1) {
    const minimumR = Math.max(
      -BUZZELLO_BOARD_RADIUS,
      -q - BUZZELLO_BOARD_RADIUS,
    );
    const maximumR = Math.min(
      BUZZELLO_BOARD_RADIUS,
      -q + BUZZELLO_BOARD_RADIUS,
    );
...
export const BUZZELLO_COORDINATES = buildCoordinates();
...
export const BUZZELLO_CORNER_INDICES = [
  [0, -4],
  [4, -4],
  [4, 0],
  [0, 4],
...
export function getBuzzelloCoordinate(index: number): BuzzelloCoordinate {
...
export function formatBuzzelloCoordinate(index: number): string {
...
export function getBuzzelloScores(board: BuzzelloBoard): BuzzelloScores {
  let yellow = 0;
  let black = 0;
...
export function isBuzzelloGameOver(board: BuzzelloBoard): boolean {
  const scores = getBuzzelloScores(board);
...
export function getBuzzelloPositionalWeight(index: number): number {
...
export function evaluateBuzzelloBoard(
  board: BuzzelloBoard,
  player: BuzzelloPlayer,
): number {
  const opponent = getBuzzelloOpponent(player);
  ...
  const parityWeight = occupiedRatio < 0.65 ? 1 : occupiedRatio < 0.85 ? 4 : 12;
...
export function selectBuzzelloAiMove(
  board: BuzzelloBoard,
  player: BuzzelloPlayer,
