...
describe.skipIf(!emulatorHost)("learning migration (Firestore emulator)", () => {
  ...
  it("applies cleanup and refreshes an already-published lesson without restaging it", async () => {
    ...
    expect((await db.doc("docs/montyhall/revisions/academy_v5_cleanup_montyhall").get()).exists).toBe(true);
    expect((await db.doc("audit_logs/academy_v5_cleanup_montyhall").get()).data()).not.toHaveProperty("content");
    ...
    expect((await db.doc("audit_logs/academy_v5_refresh-published_ftc-gui-owned-indicator-lights").get()).data()).toMatchObject({
