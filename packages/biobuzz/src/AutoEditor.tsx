...
export default function AutoEditor({program,onChange,onPreview,canRun=true}:{program:AutoProgram;onChange:(program:AutoProgram)=>void;onPreview:()=>void;canRun?:boolean}) {
  ...
  return <section className="bio-card" aria-label="Auto editor">
    ...
    <h3>Add an action</h3>
    <div className="bio-row"><button disabled={program.steps.length>=128} onClick={()=>add({kind:"drive",target:{x:0.8,y:0.9,heading:0},preset:"safe"})}>Add waypoint</button><button disabled={program.steps.length>=128} onClick={()=>add({kind:"wait",seconds:1})}>Add wait</button><button disabled={program.steps.length>=128} onClick={()=>add({kind:"intake",enabled:true})}>Add intake</button><button disabled={program.steps.length>=128} onClick={()=>add({kind:"shoot",count:1,speed:5.8})}>Add shot</button></div>
    <p className="bio-help">Steps run in order. Intake stays on through later drive and wait steps until you turn it off or the auto ends. Shoot uses the current shooter direction and the selected launch speed. Start with a waypoint to aim your robot.</p>
    ...
    <ol className="bio-steps">{program.steps.map((step,i)=><li key={i}><div className="bio-row"><strong>{i+1}. {step.kind}</strong><button type="button" aria-label={"Move step "+(i+1)+" up"} disabled={i===0} onClick={()=>move(i,-1)}>↑</button><button type="button" aria-label={"Move step "+(i+1)+" down"} disabled={i===program.steps.length-1} onClick={()=>move(i,1)}>↓</button><button type="button" aria-label={"Remove step "+(i+1)} onClick={()=>onChange({...program,steps:program.steps.filter((_,j)=>j!==i)})}>Remove</button></div>
    ...
    <p className="bio-help">Robot settings are saved with the browser auto. Turret autos start at the shooter's home direction. Studio export supports the front-facing reference robot with a fixed shooter, default drive speeds, and an intake that collects pollen and nectar.</p>
