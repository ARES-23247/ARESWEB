import type { AutoProgram, Config, Input, SeatKind, Snapshot } from "./types";
export const PROTOCOL = 1;
export interface Session { roomId:string; token:string; seat:number; code:string; socketUrl:string }
export interface Lobby { roomId:string; code:string; public:boolean; seats:SeatKind[]; occupied:boolean[]; ready:boolean[]; host:number; status:"waiting"|"running"|"local-offer"|"finished"|"interrupted"; waitSeconds:number }
