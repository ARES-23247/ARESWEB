import type { Element,MechanismSide,Robot,RobotSetup } from "./types";
...
export function validateRobotSetup(value:unknown):RobotSetup {
  ...
  if(r.intakeContents!==undefined&&!["pollen","both"].includes(r.intakeContents))throw new Error("Choose valid intake ball types.");
  ...
  return {shooter:r.shooter,deposit:r.deposit,intake:r.intake,...(r.intakeContents!==undefined?{intakeContents:r.intakeContents}:{}),...(r.turret!==undefined?{turret:r.turret}:{}),...(r.driveSpeed!==undefined?{driveSpeed:r.driveSpeed}:{}),...(r.turnSpeed!==undefined?{turnSpeed:r.turnSpeed}:{})};
}
export function canIntakeBall(robot:Pick<Robot,"setup"|"alliance">,kind:Element){return kind==="pollen"||(robot.setup.intakeContents!=="pollen"&&kind===robot.alliance);}
