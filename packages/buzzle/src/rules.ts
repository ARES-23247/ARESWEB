...
export class BuzzleRuleError extends Error {
  constructor(public readonly code: string, message: string) {
    super(message);
    this.name = "BuzzleRuleError";
  }
}
...
// Persisted online boards predate the row-major browser grid. Transposing q/r
// converts indices in either direction and preserves every existing match slot.
export const BUZZLE_ONLINE_INDICES = BUZZLE_COORDINATES.map(({ q, r }) =>
  BUZZLE_COORDINATE_INDEX.get(`${r},${q}`)!);

export const BUZZLE_LETTER_DISTRIBUTION: ReadonlyArray<
...
export function createBuzzleTileBag(): BuzzleTile[] {
  ...
  for (const [letter, count, points] of BUZZLE_LETTER_DISTRIBUTION) {
...
function normalizePlacedTile(placement: BuzzlePlacement, playedBy: number): BuzzleBoardTile {
  ...
  if (placement.tile.blank) {
    if (!assigned || !/^[A-Z]$/u.test(assigned)) {
      throw new BuzzleRuleError("BUZZLE_BLANK_REQUIRED", "Choose a letter for every blank tile.");
    }
    return { ...placement.tile, letter: assigned, points: 0, playedBy };
  }
  if (assigned && assigned !== placement.tile.letter) {
    throw new BuzzleRuleError("BUZZLE_INVALID_BLANK", "Only blank tiles can change their letter.");
  }
...
function axisForPlacements(placements: ReadonlyArray<BuzzlePlacement>): number {
  ...
  if (axis < 0) throw new BuzzleRuleError("BUZZLE_NOT_LINEAR", "Place every new tile in one straight hex line.");
  return axis;
}
...
export function analyzeBuzzlePlay(
  ...
  if (placements.length < 1 || placements.length > BUZZLE_RACK_SIZE) {
    throw new BuzzleRuleError("BUZZLE_PLACEMENT_COUNT", "Place between one and seven tiles.");
  }
  ...
  for (const placement of placements) {
    if (!Number.isSafeInteger(placement.index) || placement.index < 0 || placement.index >= BUZZLE_CELL_COUNT) {
      throw new BuzzleRuleError("BUZZLE_ILLEGAL_PLACEMENT", "A tile is outside the BUZZLE board.");
    }
    if (board[placement.index] || placementMap.has(placement.index)) {
      throw new BuzzleRuleError("BUZZLE_ILLEGAL_PLACEMENT", "Each new tile needs an empty board cell.");
    }
  ...
  if (boardIsEmpty && !placementIndices.has(center)) {
    throw new BuzzleRuleError("BUZZLE_CENTER_REQUIRED", "The opening word must cover the center star.");
  }
  if (!boardIsEmpty) {
    ...
    if (!connected) throw new BuzzleRuleError("BUZZLE_DISCONNECTED", "The new word must connect to the hive.");
  }
  ...
  if (placements.some(({ index }) => !mainWord.indices.includes(index))) {
    throw new BuzzleRuleError("BUZZLE_GAPPED_WORD", "Words cannot contain empty gaps.");
  }
  if (mainWord.indices.length < 2) throw new BuzzleRuleError("BUZZLE_GAPPED_WORD", "A play must form a word of at least two letters.");
  ...
  for (const placement of placements) {
    for (let wordAxis = 0; wordAxis < HEX_WORD_AXES.length; wordAxis += 1) {
      ...
      if (!dictionary.has(text.toLowerCase())) {
        throw new BuzzleRuleError("BUZZLE_WORD_NOT_FOUND", `${text} is not in the selected dictionary.`);
      }
      words.set(key, {
        word: text,
  ...
  if (words.size === 0) throw new BuzzleRuleError("BUZZLE_GAPPED_WORD", "The play does not form a complete word.");
...
export function playBuzzleTiles(
  ...
  if (state.finished) throw new BuzzleRuleError("BUZZLE_FINISHED", "This BUZZLE game is finished.");
  ...
  if (placements.some(({ tile }) => !rackIds.has(tile.id)) || new Set(placements.map(({ tile }) => tile.id)).size !== placements.length) {
    throw new BuzzleRuleError("BUZZLE_ILLEGAL_PLACEMENT", "Every placed tile must come from the active rack.");
  }
...
export function exchangeBuzzleTiles(
  ...
  if (tileIds.length < 1 || tileIds.length > BUZZLE_RACK_SIZE || state.bag.length < tileIds.length) {
    throw new BuzzleRuleError("BUZZLE_INVALID_EXCHANGE", "Choose available rack tiles that the bag can replace.");
  }
  const selected = new Set(tileIds);
  if (selected.size !== tileIds.length) throw new BuzzleRuleError("BUZZLE_INVALID_EXCHANGE", "Choose each tile once.");
  const players = state.players.map((entry) => ({ ...entry, rack: [...entry.rack] }));
  const player = players[state.currentPlayer];
  const returned = player.rack.filter((tile) => selected.has(tile.id));
  if (returned.length !== tileIds.length) throw new BuzzleRuleError("BUZZLE_INVALID_EXCHANGE", "Exchange only tiles from the active rack.");
  const bag = [...state.bag];
