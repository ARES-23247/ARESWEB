...
- The former mixed `interactive.spec.ts` is split into tasks, events, store and editor suites with the same cases and interaction assertions. Task and Academy navigation waits for DOM readiness followed by explicit UI assertions; it does not wait for all external avatars/fonts or unrelated SDK traffic. CI exposed navigation timeouts before those assertions, and a controlled delayed-avatar run reproduced the timeout while the task board remained usable. No behavior or platform coverage is removed. The observed mobile skip-link retry remains under investigation; this change does not claim a fix without a reproducible cause.
- Browser execution has a shorter step deadline than its job deadline so a stalled suite leaves time to upload its failure traces and inventory. A timeout still fails the required gate.
