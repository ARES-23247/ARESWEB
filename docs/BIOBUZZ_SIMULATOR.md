...
Aim (H, left trigger, or the Aim button) toggles a persistent hive lock. A fixed shooter steers the chassis toward the open alliance hive cell while preserving translation; Q/E or the right stick temporarily overrides heading, and releasing it resumes tracking. A turret tracks independently. Blocked trajectories retry after repositioning, including with an empty magazine. Lock-on selects a clear trajectory within 2–5.8 m/s and 30–80 degrees elevation. Ready requires alignment, low chassis speed, cooldown completion, and a final swept collision check. Aim never fires by itself. Holding Shoot (F or right trigger) repeats at the existing 20-tick cooldown, at most three balls per second; each locked shot waits for a fresh clear, steady aim. Release stops firing with no queued shot. With lock off, held shooting uses manual power and the current shooter direction. Neutral/stale input and period deadlines stop tracking and shooting. Flower power switches lock off.

Optional input fields `lockOn`, `shootHeld`, and `depositHeld` carry explicit actuator states. Clients without these fields retain legacy aim/shoot/placement edge behavior during a rolling release. The server validates these fields as booleans and applies the same canonical engine as local play. Legacy shots and automatic flower placement have a bounded alignment timeout of `max(5, 8 / turnSpeed)` seconds. Legacy fire clicks received within 250 ms of Ready receive a bounded geometry recheck; held shooting instead follows the live button state. Neutral input discards tracking and pending edges immediately.
...
Configure robot selects front/back hive shooting, front/back flower placement, front/back/both intake, an optional shooter turret, chassis speed, and turn speed. Both intakes share the four-ball magazine and intake interval. The turret has full rotation at a modeled 180°/s and a muzzle 0.28 m from its center; shooter side sets its home direction. Local turret practice starts with lock on; online drivers choose the lock explicitly. Brackets or bumpers temporarily override the turret. Flower placement takes priority, then hive tracking resumes. The front/back layout diagram shows each configured mechanism, and separate colored I (intake), F (flower placement), and S (fixed shooter) badges mark their sides on the field. A turret retains its rotating barrel. These are virtual reference mechanisms, with no claim of measured hardware dynamics.
...
Place in flower (G or X/Square) starts one short projectile arc through a nearby flower top using the configured placement side. A tap completes one placement; holding repeats while a ball and clear flower space are available, with the same 20-tick firing cooldown. Releasing stops starting new placements; the current placement finishes. Driving cancels it. It requires a clear arc and a flower with space within 0.95 m, retains the ball when blocked, and follows the existing capacity, ownership and early-nectar penalty rules. Intake remains separately controlled; leaving it on near a flower can retrieve pollen again.

The page reports standard gamepad connection status. Left stick translates in the selected driver view, right stick turns the chassis, A/Cross toggles intake, left trigger toggles aim lock, right trigger shoots while held, bumpers turn the turret, X/Square places in a flower while held, Y/Triangle releases nectar, and View/Share switches the driver view. Aim and intake toggle once per press; holding does not toggle repeatedly. A 0.12 stick dead zone suppresses drift. Controller disconnection neutralizes controls and turns intake and aim lock off. Unsupported mappings are reported. Browser tests substitute only the hardware API and exercise the real controls and worker; they are not physical-controller tests.

Legacy controls retain at most one pending aim/shoot/placement edge between fixed physics updates. Neutral controls, stale input, and disconnects discard pending actions. Explicit held controls resume only on fresh input and never enqueue a series of shots.

## Practice interpretation

Local solo play starts with a TELEOP countdown from 2:00. Timer mode also offers AUTO only (0:30) and Combined (2:30 of play). Choose a mode and restart the timed match to apply it. Combined runs 30 seconds AUTO, eight seconds disabled transition, then 120 seconds TELEOP; its main countdown holds at 2:00 during the transition, while a separate period readout counts the eight seconds. AUTO only settles after 30 seconds and never enables TELEOP. Online matches use the combined countdown. All modes settle for up to ten seconds after play ends, with the main timer held at 0:00. An unresolved settling or server scheduling fault marks the match incomplete. Local tab suspension and Pause stop local time. Untimed practice remains available explicitly, displays UNTIMED instead of an elapsed stopwatch, and permits nectar. At TELEOP 1:00 (98 seconds from AUTO start), the same fixed-step threshold changes the banner, adds a phase event, removes the early-nectar penalty, and permits remaining reserve release. Pollen flower scoring is legal throughout; nectar placed early still scores but incurs the V1 G410 20-point opponent penalty. Final locations continue settling after 0:00.
...
The visual editor supports drive, wait, intake on/off, aim lock on/off, and shot steps. Intake and lock start off, persist through later steps until changed, and neutralize when the routine or AUTO ends. Fixed-shooter lock overrides waypoint heading while allowing translation. Locked shot steps count actual launches, wait for alignment, and use calculated power; unlocked steps retain authored launch speed. Lock steps can be saved and run locally or online. Studio export rejects them explicitly because the native reference robot has no equivalent lock action. ZIP export contains schema-2 `.aresroutine` and schema-1 autonomous catalog documents. Mechanism macros lower to the existing RobotBuilder intake/flywheel/transfer voltage actions. Each translating waypoint exports native rotate/straight-drive/rotate steps. Studio uses heading as the path tangent, so this prevents an unwanted curved segment while preserving the requested final heading. Driving uses the selected native motion preset; the extra turns and native controller determine actual timing. Default autos start diagonally against the wall so the same pose satisfies Studio’s conservative circular sweep clearance. Native preflight may reject tighter custom paths that the rectangular browser robot can physically traverse; edit those paths before running them. Export preserves the authored coordinates.
