...
- Name: admin-users.spec.ts >> Admin Users Dashboard >> User Management dashboard loads and displays user list
- Location: tests/e2e/admin-users.spec.ts:17:3
...
```ts
  ...
  4   | import { TEST_TIMEOUTS } from '../fixtures/mock-data';
  5   | 
  6   | test.describe('Admin Users Dashboard', () => {
  7   |   test.beforeEach(async ({ page }) => {
  8   |     await setupMockAuth(page, { useRealAuth: true });
  9   |   });
  10  | 
  11  |   test.afterEach(async ({ page }) => {
  12  |     // Clean up all routes to prevent memory buildup
  13  |     await page.unrouteAll();
